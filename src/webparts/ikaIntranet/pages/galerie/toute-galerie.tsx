...
import { FaImages, FaExpand, FaXmark, FaChevronLeft, FaChevronRight, FaFolder, FaArrowLeft, FaChevronDown } from 'react-icons/fa6';
...
export const TouteGalerie: React.FC<{ siteUrl?: string }> = ({ siteUrl }) => {
  ...
  const [rootImages, setRootImages] = React.useState<IGalerieImage[]>([]);
  const [rootVisible, setRootVisible] = React.useState(PAGE_SIZE);
  const [loadingRootImages, setLoadingRootImages] = React.useState(true);
  const [images, setImages] = React.useState<IGalerieImage[]>([]);
  const [folderVisible, setFolderVisible] = React.useState(PAGE_SIZE);
  ...
  React.useEffect(() => {
    if (!siteUrl) {
      ...
      setLoadingRootImages(false);
      return;
    }
    getGalerieRootFolder(siteUrl).then((url) => {
      loadGalerieFolders(siteUrl, url)
        .then((f) => {
          setFolders(f);
          setLoadingFolders(false);
        })
      ...
      loadGalerieImages(siteUrl, url)
        .then((imgs) => {
          setRootImages(imgs);
          setLoadingRootImages(false);
        })
        .catch((err) => {
          console.error('[TouteGalerie] Images racine :', err);
          setLoadingRootImages(false);
        });
    }).catch((err) => {
      console.error('[TouteGalerie] Dossier racine :', err);
      setLoadingFolders(false);
      setLoadingRootImages(false);
    });
  }, [siteUrl]);
  ...
  const displayedImages = currentFolder ? images : rootImages;

  React.useEffect(() => {
    const onKey = (e: KeyboardEvent): void => {
      if (lightbox === null) return;
      if (e.key === 'ArrowLeft') setLightbox((lightbox + displayedImages.length - 1) % displayedImages.length);
      if (e.key === 'ArrowRight') setLightbox((lightbox + 1) % displayedImages.length);
      if (e.key === 'Escape') setLightbox(null);
    };
    window.addEventListener('keydown', onKey);
    return (): void => window.removeEventListener('keydown', onKey);
  }, [lightbox, displayedImages.length]);
  ...
  return (
    <main className="pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800">
      <div className="mx-auto max-w-[1650px] px-4 sm:px-6 lg:px-8 space-y-4">
        ...
        {/* Contenu : images hors dossier */}
        {!currentFolder && !loadingRootImages && rootImages.length > 0 && (
          <div className="space-y-3">
            <span className="text-xs font-bold text-slate-600 flex items-center gap-1.5">
              <FaImages className="text-purple-500" /> Photos ({rootImages.length})
            </span>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
              {rootImages.slice(0, rootVisible).map((g, i) => (
                <button
                  key={g.id}
            ...
            {rootVisible < rootImages.length && (
              <button
                onClick={() => setRootVisible((v) => v + PAGE_SIZE)}
                className="mx-auto flex items-center gap-1.5 px-4 py-2 rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 transition text-xs font-bold"
              >
                Afficher plus <FaChevronDown className="text-[10px]" />
              </button>
            )}
          </div>
        )}
          ...
          <>
            ...
            {loadingImages ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {images.slice(0, folderVisible).map((g, i) => (
                  <button
                    key={g.id}
                    onClick={() => setLightbox(i)}
                    className="group relative rounded-xl overflow-hidden aspect-video bg-slate-900 cursor-pointer shadow"
            ...
            {folderVisible < images.length && (
              <button
                onClick={() => setFolderVisible((v) => v + PAGE_SIZE)}
                className="mx-auto flex items-center gap-1.5 px-4 py-2 rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 transition text-xs font-bold"
              >
                Afficher plus <FaChevronDown className="text-[10px]" />
              </button>
      ...
      {lightbox !== null && displayedImages[lightbox] && (
        ...
        >
          <div className="absolute top-4 left-0 right-0 flex items-center justify-between px-6 z-10">
            <span className="text-white/70 text-xs font-semibold bg-black/40 px-3 py-1.5 rounded-full">{lightbox + 1} / {displayedImages.length}</span>
          ...
          <div className="relative w-full max-w-4xl flex items-center justify-center">
            <button onClick={() => setLightbox((lightbox + displayedImages.length - 1) % displayedImages.length)} className="absolute left-0 z-10 w-11 h-11 rounded-full bg-white/10 hover:bg-white/25 border border-white/20 text-white flex items-center justify-center transition -translate-x-2">
              <FaChevronLeft />
            </button>
            <img src={displayedImages[lightbox].url} alt="" className="max-h-[75vh] max-w-full object-contain rounded-xl shadow-2xl" loading="lazy" />
            <button onClick={() => setLightbox((lightbox + 1) % displayedImages.length)} className="absolute right-0 z-10 w-11 h-11 rounded-full bg-white/10 hover:bg-white/25 border border-white/20 text-white flex items-center justify-center transition translate-x-2">
              <FaChevronRight />
            </button>
          </div>
          <div className="mt-4 text-center">
            <p className="text-white font-bold text-sm">{displayedImages[lightbox].title}</p>
            {displayedImages[lightbox].description && (
              <p className="text-white/60 text-xs mt-1 max-w-xl">{displayedImages[lightbox].description}</p>
            )}
          </div>
        </div>
      )}
