...
import {
  IAbsence, IAbsencePayload, ABSENCE_TYPES, TypeAbsence, loadAbsence, createAbsence, updateAbsence,
  loadAbsenceAttachment, uploadAbsenceAttachment, removeAbsenceAttachment
} from '../../../services/workflow/absences/index';
...
const emptyForm: IFormState = { titre: '', type: ABSENCE_TYPES[0], dateDebut: '', dateFin: '', jours: '', motif: '', validateurEmail: '' };

function formFromExisting(existing: IAbsence): IFormState {
  return {
...
export const AjouterAbsence: React.FC<IAjouterAbsenceProps> = (props) => {
  ...
  const [original, setOriginal] = React.useState<IAbsence | undefined>(undefined);
    ...
    if (isEdit && id) {
      ...
      loadAbsence(siteUrl, id)
        .then((existing) => {
          ...
          setLoading(false);
          loadAbsenceAttachment(siteUrl, id).then((att) => { if (!cancelled) setExistingAttachment(att); }).catch(() => undefined);
        })
  ...
  const handleSubmit = (e: React.FormEvent): void => {
    ...
    const afterSave = (ok: boolean, targetId?: number): void => {
      if (ok && targetId && file) {
        uploadAbsenceAttachment(siteUrl, targetId, file)
          .then((uploaded) => { if (!uploaded) setAttachmentWarning("Le signalement a été enregistré mais l'envoi de la pièce jointe a échoué."); finish(ok, targetId); })
          .catch(() => { setAttachmentWarning("Le signalement a été enregistré mais l'envoi de la pièce jointe a échoué."); finish(ok, targetId); });
        return;
      }
      finish(ok, targetId);
    };
    ...
    if (isEdit && id) {
      updateAbsence(siteUrl, id, payload).then((ok) => afterSave(ok, id)).catch(() => finish(false));
    } else {
      createAbsence(siteUrl, payload).then((createdId) => afterSave(!!createdId, createdId)).catch(() => finish(false));
    }
  };
        ...
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-2xl p-10 shadow-sm border border-slate-200 text-center">
            ...
            <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
              {newId ? (
                <a href={`#page-workflow-detail-absence&id=${newId}`} className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-ikaBlue text-white font-bold text-xs hover:bg-blue-600 shadow transition">
                  Voir le signalement
                </a>
              ) : null}
    ...
    <main className="pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800">
      <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 space-y-4">
        <nav className="flex items-center gap-2 text-[11px] font-semibold text-slate-500 flex-wrap">
          ...
          <span>/</span>
          <span className="text-ikaBlue">{isEdit ? `Modifier : ${form.titre || ''}` : "Nouveau signalement d'absence"}</span>
        </nav>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="px-6 sm:px-8 py-5 border-b border-slate-100">
            ...
            <p className="mt-1 text-xs text-slate-500">Le signalement sera transmis au validateur indiqué.</p>
          </div>
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-5">
            ...
            <div>
              ...
              <input type="text" value={form.titre} onChange={(e) => setField('titre', e.target.value)} required placeholder="Titre du signalement" className={`${inputCls} ${errors.titre ? 'border-rose-300' : ''}`} />
            ...
            <div>
              ...
              <select value={form.type} onChange={(e) => setField('type', e.target.value as TypeAbsence)} className={inputCls}>
            ...
            <div>
              ...
              <textarea value={form.motif} onChange={(e) => setField('motif', e.target.value)} required rows={4} placeholder="Décrivez le motif de votre absence..." className={`${inputCls} ${errors.motif ? 'border-rose-300' : ''}`} />
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <button type="submit" disabled={saving} className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-ikaBlue text-white font-bold text-xs hover:bg-blue-600 shadow transition disabled:opacity-60 disabled:cursor-not-allowed">
                <FaPaperPlane /> {saving ? 'Enregistrement...' : (isEdit ? 'Enregistrer les modifications' : 'Envoyer le signalement')}
              </button>
              <a
