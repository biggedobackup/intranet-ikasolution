...
import { CongeStatus, IConge, loadConges, deleteConge, formatDateFR } from '../../../services/workflow/conges/index';
import { Pagination } from '../../../components/Pagination';
import { ConfirmDelete } from '../../../components/ConfirmDelete';

export interface IListeCongeProps {
  siteUrl?: string;
}

const statusBadge = (status: CongeStatus): React.ReactElement => {
  switch (status) {
...
export const ListeConge: React.FC<IListeCongeProps> = (props) => {
  const { siteUrl } = props;
  const [search, setSearch] = React.useState('');
  const [status, setStatus] = React.useState('all');
  const [page, setPage] = React.useState(1);
  const [items, setItems] = React.useState<IConge[]>([]);
  const [loading, setLoading] = React.useState<boolean>(true);
  const [error, setError] = React.useState<string>('');
  const [deleteItem, setDeleteItem] = React.useState<IConge | null>(null);
  const [deleting, setDeleting] = React.useState<boolean>(false);

  const fetchItems = React.useCallback((force?: boolean): void => {
    if (!siteUrl) { setLoading(false); return; }
    setLoading(true);
    loadConges(siteUrl, force)
      .then((data) => { setItems(data); setLoading(false); })
      .catch(() => { setError('Impossible de charger les demandes de congé.'); setLoading(false); });
  }, [siteUrl]);
  ...
  const filtered = items.filter((i) => {
    ...
    const matchesSearch = i.titre.toLowerCase().indexOf(q) !== -1 || i.demandeur.toLowerCase().indexOf(q) !== -1 || i.motif.toLowerCase().indexOf(q) !== -1;
  ...
  const handleConfirmDelete = (): void => {
    ...
    setDeleting(true);
    deleteConge(siteUrl, deleteItem.id)
      .then((ok) => {
        setDeleting(false);
        if (ok) {
    ...
    <main className="pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800">
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-200 relative overflow-hidden">
          ...
          <div className="relative">
            <nav className="flex items-center gap-2 text-[11px] font-semibold text-slate-500 flex-wrap">
              ...
              <span>/</span>
              <span className="text-ikaBlue">Demandes de Congé</span>
            </nav>
            <div className="mt-3 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h1 className="text-2xl sm:text-3xl font-black text-ikaBlueDark">Demandes de Congé</h1>
                <p className="mt-2 text-sm text-slate-500 max-w-2xl">
                  Consultez, ajoutez ou modifiez les demandes de congé.
                </p>
              </div>
              <a
                href="#page-workflow-ajouter-conge"
              ...
              >
                <FaPlus /> Nouvelle demande de congé
              </a>
            </div>
                    ...
                    <tr key={i.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="inline-flex items-center gap-1.5">
                          <a
                            href={`#page-workflow-detail-conge&id=${i.id}`}
                          ...
                          <a
                            href={`#page-workflow-modifier-conge&id=${i.id}`}
      ...
      {deleteItem ? (
        <ConfirmDelete
          ...
          message={`Voulez-vous vraiment supprimer la demande de congé « ${deleteItem.titre} » de ${deleteItem.demandeur} ? Cette action est irréversible.`}
