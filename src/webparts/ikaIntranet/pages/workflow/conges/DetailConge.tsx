...
import { CONGES, IConge } from '../../../services/workflow/conges/data';
import { CONGE_DECISION_CONFIG, applyCongeDecision, DecisionAction } from '../../../services/workflow/conges/DecisionValidation';
...
export const DetailConge: React.FC = () => {
  const [conge, setConge] = React.useState<IConge>(() => CONGES.find((c) => c.id === getCongeIdFromHash()) || CONGES[0]);
  const [decision, setDecision] = React.useState<DecisionAction | null>(null);

  React.useEffect(() => {
    const onHash = (): void => {
      const id = getCongeIdFromHash();
      setConge(CONGES.find((c) => c.id === id) || CONGES[0]);
      setDecision(null);
    };
  ...
  const isEnAttente = conge.statut === 'En attente';

  const handleDecision = (comment: string, date: string): void => {
    if (!decision) return;
    setConge((prev) => applyCongeDecision(prev, decision, comment, date));
    setDecision(null);
  };
    ...
    <main className="pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800">
          ...
          <div className="p-6 sm:p-8 space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="rounded-xl border border-slate-100 p-4 bg-slate-50/60">
                ...
                <p className="mt-1.5 text-sm font-bold text-slate-800">{conge.dateDebut} → {conge.dateFin}</p>
              </div>
              <div className="rounded-xl border border-slate-100 p-4 bg-slate-50/60">
            ...
            {isEnAttente ? (
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-5 space-y-2">
                ...
                <p className="text-xs font-semibold text-slate-700">
                  ...
                  {conge.decisionDate && <span className="text-slate-400 font-normal"> — le {conge.decisionDate}</span>}
                </p>
                <p className="text-xs text-slate-600 leading-relaxed">{conge.decisionComment}</p>
              </div>
            ) : null}

            <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] font-semibold text-slate-400">
              <span>Créée le {conge.createdAt}</span>
            </div>
      ...
      {decision && (
        <DecisionModal
          ...
          actionLabel={decision === 'valider' ? CONGE_DECISION_CONFIG.validateLabel : CONGE_DECISION_CONFIG.rejectLabel}
          action={decision}
