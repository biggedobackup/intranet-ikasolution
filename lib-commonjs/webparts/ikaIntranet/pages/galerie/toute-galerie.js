...
var index_1 = require("../../services/galerie/index");
var TouteGalerie = function (_a) {
    ...
    React.useEffect(function () {
        ...
        (0, index_1.getGalerieRootFolder)(siteUrl)
            .then(function (rootUrl) { return (0, index_1.loadGalerieFolders)(siteUrl, rootUrl); })
            .then(function (f) {
            setFolders(f);
            setLoadingFolders(false);
        })
    ...
    var openFolder = function (folder) {
        ...
        setImages([]);
        (0, index_1.loadGalerieImages)(siteUrl, folder.serverRelativeUrl)
            .then(function (imgs) {
            setImages(imgs);
            ...
            currentFolder && (React.createElement(React.Fragment, null,
                React.createElement("div", { className: "flex items-center gap-3 flex-wrap" },
                    React.createElement("button", { onClick: goBack, className: "px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 transition text-xs font-bold flex items-center gap-1.5" },
                        React.createElement(fa6_1.FaArrowLeft, { className: "text-[10px]" }),
                        " Tous les dossiers"),
                    React.createElement("span", { className: "text-xs font-bold text-slate-600 flex items-center gap-1.5" },
                        React.createElement(fa6_1.FaFolder, { className: "text-purple-500" }),
                        " ",
                        currentFolder.name),
