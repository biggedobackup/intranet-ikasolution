...
var DetailConge = function (props) {
    ...
    var fetchConge = React.useCallback(function () {
        ...
        (0, index_1.loadConge)(siteUrl, id)
            ...
            setLoading(false);
            if (item)
                (0, index_1.loadCongeAttachment)(siteUrl, item.id).then(setAttachment).catch(function () { return undefined; });
        })
    ...
    var handleDecision = function (comment, date) {
        if (!decision || !siteUrl || !conge)
            return;
        setDeciding(true);
        (0, index_1.applyCongeDecision)(siteUrl, conge, decision, comment, date)
            .then(function (ok) {
            setDeciding(false);
            if (ok) {
                setDecision(null);
    ...
    var handleDelete = function () {
        if (!siteUrl || !conge)
            return;
        setDeleting(true);
        (0, index_1.deleteConge)(siteUrl, conge.id)
            .then(function (ok) {
            setDeleting(false);
            if (ok)
            ...
            React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" },
                ...
                React.createElement("div", { className: "p-6 sm:p-8 space-y-5" },
                    React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4" },
                        ...
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                            ...
                            React.createElement("p", { className: "mt-1.5 text-sm font-bold text-slate-800" },
                                (0, index_1.formatDateFR)(conge.dateDebut),
                                " \u2192 ",
                                (0, index_1.formatDateFR)(conge.dateFin))),
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                            React.createElement("span", { className: "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400" },
                                React.createElement(fa6_1.FaCalendarDays, { className: "text-ikaBlue" }),
                                " Type de cong\u00E9"),
                    ...
                    isEnAttente && isValidateur ? (React.createElement("div", { className: "rounded-xl border border-amber-200 bg-amber-50/60 p-5 space-y-3" },
                        ...
                        React.createElement("p", { className: "text-xs font-semibold text-slate-700" },
                            ...
                            conge.dateDecision ? React.createElement("span", { className: "text-slate-400 font-normal" },
                                " \u2014 le ",
                                (0, index_1.formatDateFR)(conge.dateDecision)) : null),
                    ...
                    React.createElement("div", { className: "pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] font-semibold text-slate-400" },
                        React.createElement("span", null,
                            "Cr\u00E9\u00E9e le ",
                            (0, index_1.formatDateFR)(conge.createdAt))),
                    React.createElement("div", { className: "pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3" },
                        ...
                        React.createElement("a", { href: "#page-workflow-liste-conge", className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:bg-slate-50 transition" },
                            React.createElement(fa6_1.FaArrowLeft, null),
                            " Retour \u00E0 la liste"))))),
        decision ? (React.createElement(DecisionModal_1.DecisionModal, { title: DecisionValidation_1.CONGE_DECISION_CONFIG.modalTitle(decision), message: DecisionValidation_1.CONGE_DECISION_CONFIG.modalMessage(conge, decision), actionLabel: deciding ? 'Enregistrement...' : (decision === 'valider' ? DecisionValidation_1.CONGE_DECISION_CONFIG.validateLabel : DecisionValidation_1.CONGE_DECISION_CONFIG.rejectLabel), action: decision, onConfirm: handleDecision, onCancel: function () { return setDecision(null); } })) : null,
        confirmDelete ? (React.createElement(ConfirmDelete_1.ConfirmDelete, { title: "Supprimer la demande", message: "Voulez-vous vraiment supprimer la demande de cong\u00E9 \u00AB ".concat(conge.titre, " \u00BB de ").concat(conge.demandeur, " ? Cette action est irr\u00E9versible."), onConfirm: handleDelete, onCancel: function () { return !deleting && setConfirmDelete(false); } })) : null));
};
