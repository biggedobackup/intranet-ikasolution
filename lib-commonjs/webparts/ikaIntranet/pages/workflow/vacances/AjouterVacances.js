...
exports.AjouterVacances = void 0;
...
var FileAttachmentField_1 = require("../../../components/FileAttachmentField");
...
function validateForm(form, demandeurEmail) {
    var errors = {};
    if ((0, index_2.isBlank)(form.titre))
        errors.titre = 'Le titre est obligatoire.';
    if ((0, index_2.isBlank)(form.validateurEmail)) {
        errors.validateurEmail = 'Veuillez sélectionner un validateur.';
    }
    else if (!(0, index_2.isValidEmail)(form.validateurEmail)) {
    ...
    if ((0, index_2.isBlank)(form.destination))
    ...
    if ((0, index_2.isBlank)(form.motif))
...
var AjouterVacances = function (props) {
    ...
    React.useEffect(function () {
        setForm(function (prev) {
            var computed = (0, index_2.computeJoursInclusive)(prev.dateDebut, prev.dateFin);
            var next = computed !== undefined ? String(computed) : '';
            return next === prev.jours ? prev : tslib_1.__assign(tslib_1.__assign({}, prev), { jours: next });
        });
    ...
    React.useEffect(function () {
        ...
        if (isEdit && id) {
            ...
            (0, index_1.loadVacance)(siteUrl, id)
                ...
                setLoading(false);
                (0, index_1.loadVacanceAttachment)(siteUrl, id).then(function (att) { if (!cancelled)
            ...
            (0, index_2.getCurrentUserName)(siteUrl).then(function (name) { if (!cancelled)
                setDemandeurNom(name); }).catch(function () { return undefined; });
            (0, index_2.getCurrentUserEmail)(siteUrl).then(function (email) { if (!cancelled)
    ...
    var setField = function (key, value) {
        ...
        setErrors(function (prev) {
            var _a;
            return (prev[key] ? tslib_1.__assign(tslib_1.__assign({}, prev), (_a = {}, _a[key] = undefined, _a)) : prev);
        });
    };
    var handleRemoveAttachment = function () {
        if (!siteUrl || !id || !existingAttachment)
            return;
        (0, index_1.removeVacanceAttachment)(siteUrl, id, existingAttachment.fileName)
    ...
    var handleSubmit = function (e) {
        ...
        var afterSave = function (ok, targetId) {
            if (ok && targetId && file) {
                (0, index_1.uploadVacanceAttachment)(siteUrl, targetId, file)
        ...
        if (isEdit && id) {
            (0, index_1.updateVacance)(siteUrl, id, payload).then(function (ok) { return afterSave(ok, id); }).catch(function () { return finish(false); });
        }
        else {
            (0, index_1.createVacance)(siteUrl, payload).then(function (createdId) { return afterSave(!!createdId, createdId); }).catch(function () { return finish(false); });
        }
    };
            ...
            React.createElement("div", { className: "mx-auto max-w-2xl px-4 sm:px-6 lg:px-8" },
                React.createElement("div", { className: "bg-white rounded-2xl p-10 shadow-sm border border-slate-200 text-center" },
                    ...
                    React.createElement("div", { className: "mt-6 flex flex-col sm:flex-row items-center justify-center gap-3" },
                        newId ? (React.createElement("a", { href: "#page-workflow-detail-vacances&id=".concat(newId), className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-ikaBlue text-white font-bold text-xs hover:bg-blue-600 shadow transition" }, "Voir la demande")) : null,
    ...
    return (React.createElement("main", { className: "pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800" },
        React.createElement("div", { className: "mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 space-y-4" },
            React.createElement("nav", { className: "flex items-center gap-2 text-[11px] font-semibold text-slate-500 flex-wrap" },
                ...
                React.createElement("a", { href: "#page-workflow-liste-vacances", className: "hover:text-ikaBlue transition" }, "Vacances"),
                React.createElement("span", null, "/"),
                React.createElement("span", { className: "text-ikaBlue" }, isEdit ? "Modifier : ".concat(form.titre || '') : 'Nouvelle demande de vacances')),
            React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" },
                React.createElement("div", { className: "px-6 sm:px-8 py-5 border-b border-slate-100" },
                    React.createElement("h1", { className: "text-xl sm:text-2xl font-black text-ikaBlueDark" }, isEdit ? 'Modifier la demande de vacances' : 'Nouvelle demande de vacances'),
                ...
                React.createElement("form", { onSubmit: handleSubmit, className: "p-6 sm:p-8 space-y-5" },
                    ...
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5" },
                            ...
                            " Validateur"),
                        React.createElement(UserPicker_1.UserPicker, { siteUrl: siteUrl, value: form.validateurEmail, onChange: function (email) { return setField('validateurEmail', email); }, placeholder: "Rechercher un collaborateur...", className: "".concat(inputCls, " ").concat(errors.validateurEmail ? 'border-rose-300' : '') }),
                    ...
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5" },
                            React.createElement(fa6_1.FaLocationDot, { className: "text-ikaBlue text-[10px]" }),
                            " Destination"),
                        React.createElement("input", { type: "text", value: form.destination, onChange: function (e) { return setField('destination', e.target.value); }, required: true, placeholder: "Lieu de vacances", className: "".concat(inputCls, " ").concat(errors.destination ? 'border-rose-300' : '') }),
                        errors.destination ? React.createElement("p", { className: "mt-1 text-[11px] font-semibold text-rose-600" }, errors.destination) : null),
                    ...
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5" },
                            React.createElement(fa6_1.FaPaperclip, { className: "text-ikaBlue text-[10px]" }),
                            " Pi\u00E8ce jointe (optionnel)"),
                        React.createElement(FileAttachmentField_1.FileAttachmentField, { existing: existingAttachment, onRemoveExisting: handleRemoveAttachment, file: file, onFileChange: setFile, className: inputCls })),
...
exports.AjouterVacances = AjouterVacances;
exports.default = exports.AjouterVacances;
//# sourceMappingURL=AjouterVacances.js.map