...
exports.DetailVacances = void 0;
var tslib_1 = require("tslib");
var React = tslib_1.__importStar(require("react"));
var fa6_1 = require("react-icons/fa6");
var index_1 = require("../../../services/workflow/vacances/index");
var DecisionValidation_1 = require("../../../services/workflow/vacances/DecisionValidation");
...
var getVacanceIdFromHash = function () {
...
var DetailVacances = function (props) {
    var siteUrl = props.siteUrl;
    var _a = React.useState(undefined), vacance = _a[0], setVacance = _a[1];
    ...
    var fetchVacance = React.useCallback(function () {
        if (!siteUrl)
            return;
        setLoading(true);
        var id = getVacanceIdFromHash();
        (0, index_1.loadVacance)(siteUrl, id)
            .then(function (item) {
            setVacance(item);
            setLoading(false);
            if (item)
                (0, index_1.loadVacanceAttachment)(siteUrl, item.id).then(setAttachment).catch(function () { return undefined; });
        })
            .catch(function () { setError('Impossible de charger la demande.'); setLoading(false); });
    }, [siteUrl]);
    React.useEffect(function () {
        fetchVacance();
        var onHash = function () { setDecision(null); fetchVacance(); };
        window.addEventListener('hashchange', onHash);
        return function () { return window.removeEventListener('hashchange', onHash); };
    }, [fetchVacance]);
    var isEnAttente = vacance && vacance.statut === 'En attente';
    var isValidateur = !!currentUserEmail && !!(vacance === null || vacance === void 0 ? void 0 : vacance.validateurEmail) && currentUserEmail.toLowerCase() === vacance.validateurEmail.toLowerCase();
    var handleDecision = function (comment, date) {
        if (!decision || !siteUrl || !vacance)
            return;
        setDeciding(true);
        (0, index_1.applyVacanceDecision)(siteUrl, vacance, decision, comment, date)
            .then(function (ok) {
            setDeciding(false);
            if (ok) {
                setDecision(null);
    ...
    var handleDelete = function () {
        if (!siteUrl || !vacance)
            return;
        setDeleting(true);
        (0, index_1.deleteVacance)(siteUrl, vacance.id)
            .then(function (ok) {
            setDeleting(false);
            if (ok)
                window.location.hash = '#page-workflow-liste-vacances';
            else {
        ...
        return (React.createElement("main", { className: "pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800" },
            React.createElement("div", { className: "mx-auto max-w-2xl px-4 sm:px-6 lg:px-8" },
                React.createElement("div", { className: "bg-white rounded-2xl p-10 shadow-sm border border-slate-200 text-center" },
                    ...
                    React.createElement("a", { href: "#page-workflow-liste-vacances", className: "mt-4 inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-ikaBlue text-white font-bold text-xs hover:bg-blue-600 shadow transition" },
    ...
    return (React.createElement("main", { className: "pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800" },
        React.createElement("div", { className: "mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 space-y-4" },
            React.createElement("nav", { className: "flex items-center gap-2 text-[11px] font-semibold text-slate-500 flex-wrap" },
                ...
                React.createElement("a", { href: "#page-workflow-liste-vacances", className: "hover:text-ikaBlue transition" }, "Demandes de Vacances"),
                React.createElement("span", null, "/"),
                React.createElement("span", { className: "text-ikaBlue" }, vacance.titre)),
            ...
            React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" },
                    ...
                    React.createElement("div", { className: "relative" },
                        React.createElement("div", { className: "flex items-start justify-between gap-4 flex-wrap" },
                            React.createElement("div", null,
                                React.createElement("h1", { className: "text-xl sm:text-2xl font-black text-ikaBlueDark" }, vacance.titre),
                ...
                React.createElement("div", { className: "p-6 sm:p-8 space-y-5" },
                    React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4" },
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                            React.createElement("span", { className: "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400" },
                                ...
                                " Demandeur"),
                            React.createElement("p", { className: "mt-1.5 text-sm font-bold text-slate-800" }, vacance.demandeur || '—')),
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                            React.createElement("span", { className: "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400" },
                                React.createElement(fa6_1.FaPlaneDeparture, { className: "text-ikaBlue" }),
                                " Destination"),
                            React.createElement("p", { className: "mt-1.5 text-sm font-bold text-slate-800" }, vacance.destination)),
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                            React.createElement("span", { className: "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400" },
                            ...
                            React.createElement("p", { className: "mt-1.5 text-sm font-bold text-slate-800" },
                                (0, index_1.formatDateFR)(vacance.dateDebut),
                                " \u2192 ",
                                (0, index_1.formatDateFR)(vacance.dateFin))),
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                            React.createElement("span", { className: "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400" },
                                React.createElement(fa6_1.FaCalendarDays, { className: "text-ikaBlue" }),
                                " Jours"),
                    ...
                    React.createElement("section", null,
                        ...
                        React.createElement("p", { className: "mt-2 text-sm leading-relaxed text-slate-600" }, vacance.motif)),
                    ...
                    isEnAttente && isValidateur ? (React.createElement("div", { className: "rounded-xl border border-amber-200 bg-amber-50/60 p-5 space-y-3" },
                        ...
                        React.createElement("p", { className: "text-xs text-slate-600 leading-relaxed" },
                            ...
                            vacance.validateur || 'le validateur désigné',
                            "."))) : vacance.commentaireDecision ? (React.createElement("div", { className: "rounded-xl border border-slate-100 bg-slate-50/60 p-5 space-y-2" },
                        React.createElement("h2", { className: "flex items-center gap-2 text-sm font-black uppercase tracking-wider text-slate-900" },
                        ...
                        React.createElement("p", { className: "text-xs font-semibold text-slate-700" },
                            vacance.statut === 'Approuvé' ? (React.createElement("span", { className: "flex items-center gap-1.5 text-emerald-700" },
                                React.createElement(fa6_1.FaCircleCheck, null),
                                " ",
                                DecisionValidation_1.VACANCE_DECISION_CONFIG.validateVerb)) : (React.createElement("span", { className: "flex items-center gap-1.5 text-rose-600" },
                                React.createElement(fa6_1.FaCircleXmark, null),
                                " ",
                                DecisionValidation_1.VACANCE_DECISION_CONFIG.rejectVerb)),
                            vacance.dateDecision ? React.createElement("span", { className: "text-slate-400 font-normal" },
                                " \u2014 le ",
                                (0, index_1.formatDateFR)(vacance.dateDecision)) : null),
                        React.createElement("p", { className: "text-xs text-slate-600 leading-relaxed" }, vacance.commentaireDecision))) : null,
                    React.createElement("div", { className: "pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] font-semibold text-slate-400" },
                        React.createElement("span", null,
                            "Cr\u00E9\u00E9e le ",
                            (0, index_1.formatDateFR)(vacance.createdAt))),
                    React.createElement("div", { className: "pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3" },
                        React.createElement("a", { href: "#page-workflow-modifier-vacances&id=".concat(vacance.id), className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-ikaBlue text-white font-bold text-xs hover:bg-blue-600 shadow transition" },
                        ...
                        React.createElement("button", { onClick: function () { return setConfirmDelete(true); }, className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-rose-200 bg-rose-50 text-rose-600 font-bold text-xs hover:bg-rose-100 transition" },
                            ...
                            " Supprimer"),
                        React.createElement("a", { href: "#page-workflow-liste-vacances", className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:bg-slate-50 transition" },
                            React.createElement(fa6_1.FaArrowLeft, null),
                            " Retour \u00E0 la liste"))))),
        decision ? (React.createElement(DecisionModal_1.DecisionModal, { title: DecisionValidation_1.VACANCE_DECISION_CONFIG.modalTitle(decision), message: DecisionValidation_1.VACANCE_DECISION_CONFIG.modalMessage(vacance, decision), actionLabel: deciding ? 'Enregistrement...' : (decision === 'valider' ? DecisionValidation_1.VACANCE_DECISION_CONFIG.validateLabel : DecisionValidation_1.VACANCE_DECISION_CONFIG.rejectLabel), action: decision, onConfirm: handleDecision, onCancel: function () { return setDecision(null); } })) : null,
        confirmDelete ? (React.createElement(ConfirmDelete_1.ConfirmDelete, { title: "Supprimer la demande", message: "Voulez-vous vraiment supprimer la demande de vacances \u00AB ".concat(vacance.titre, " \u00BB de ").concat(vacance.demandeur, " ? Cette action est irr\u00E9versible."), onConfirm: handleDelete, onCancel: function () { return !deleting && setConfirmDelete(false); } })) : null));
};
exports.DetailVacances = DetailVacances;
exports.default = exports.DetailVacances;
//# sourceMappingURL=DetailVacances.js.map