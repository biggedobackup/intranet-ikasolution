...
var DetailVacances = function () {
    var _a = React.useState(function () { return data_1.VACANCES.find(function (v) { return v.id === getVacanceIdFromHash(); }) || data_1.VACANCES[0]; }), vacance = _a[0], setVacance = _a[1];
    var _b = React.useState(null), decision = _b[0], setDecision = _b[1];
    React.useEffect(function () {
        var onHash = function () {
            var id = getVacanceIdFromHash();
            setVacance(data_1.VACANCES.find(function (v) { return v.id === id; }) || data_1.VACANCES[0]);
            setDecision(null);
        };
        window.addEventListener('hashchange', onHash);
        return function () { return window.removeEventListener('hashchange', onHash); };
    }, []);
    var isEnAttente = vacance.statut === 'En attente';
    var handleDecision = function (comment, date) {
        if (!decision)
            return;
        setVacance(function (prev) { return (0, DecisionValidation_1.applyVacanceDecision)(prev, decision, comment, date); });
        setDecision(null);
    };
                ...
                React.createElement("div", { className: "p-6 sm:p-8 space-y-5" },
                    ...
                    isEnAttente ? (React.createElement("div", { className: "rounded-xl border border-amber-200 bg-amber-50/60 p-5 space-y-3" },
                        ...
                        React.createElement("div", { className: "flex flex-col sm:flex-row items-stretch sm:items-center gap-3" },
                            ...
                            React.createElement("button", { onClick: function () { return setDecision('rejeter'); }, className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-rose-500 text-white font-bold text-xs hover:bg-rose-600 shadow transition" },
                                React.createElement(fa6_1.FaCircleXmark, null),
                                " Rejeter")))) : vacance.decisionComment ? (React.createElement("div", { className: "rounded-xl border border-slate-100 bg-slate-50/60 p-5 space-y-2" },
                        ...
                        React.createElement("p", { className: "text-xs font-semibold text-slate-700" },
                            ...
                            vacance.decisionDate && React.createElement("span", { className: "text-slate-400 font-normal" },
                                " \u2014 le ",
                                vacance.decisionDate)),
                        React.createElement("p", { className: "text-xs text-slate-600 leading-relaxed" }, vacance.decisionComment))) : null,
                    ...
                    React.createElement("div", { className: "pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3" },
                        ...
                        React.createElement("a", { href: "#page-workflow-liste-vacances", className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:bg-slate-50 transition" },
                            ...
                            " Retour \u00E0 la liste"))))),
        decision && (React.createElement(DecisionModal_1.DecisionModal, { title: DecisionValidation_1.VACANCE_DECISION_CONFIG.modalTitle(decision), message: DecisionValidation_1.VACANCE_DECISION_CONFIG.modalMessage(vacance, decision), actionLabel: decision === 'valider' ? DecisionValidation_1.VACANCE_DECISION_CONFIG.validateLabel : DecisionValidation_1.VACANCE_DECISION_CONFIG.rejectLabel, action: decision, onConfirm: handleDecision, onCancel: function () { return setDecision(null); } }))));
};
