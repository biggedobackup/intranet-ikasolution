...
function validateForm(form, demandeurEmail) {
    ...
    if ((0, index_2.isBlank)(form.description))
...
var AjouterBesoin = function (props) {
    ...
    React.useEffect(function () {
        ...
        if (isEdit && id) {
            ...
            (0, index_1.loadBesoin)(siteUrl, id)
                ...
                setLoading(false);
                (0, index_1.loadBesoinAttachment)(siteUrl, id).then(function (att) { if (!cancelled)
    ...
    var handleRemoveAttachment = function () {
        if (!siteUrl || !id || !existingAttachment)
            return;
        (0, index_1.removeBesoinAttachment)(siteUrl, id, existingAttachment.fileName)
    ...
    var handleSubmit = function (e) {
        ...
        var afterSave = function (ok, targetId) {
            if (ok && targetId && file) {
                (0, index_1.uploadBesoinAttachment)(siteUrl, targetId, file)
        ...
        if (isEdit && id) {
            (0, index_1.updateBesoin)(siteUrl, id, payload).then(function (ok) { return afterSave(ok, id); }).catch(function () { return finish(false); });
        }
        else {
            (0, index_1.createBesoin)(siteUrl, payload).then(function (createdId) { return afterSave(!!createdId, createdId); }).catch(function () { return finish(false); });
        }
    };
            ...
            React.createElement("div", { className: "mx-auto max-w-2xl px-4 sm:px-6 lg:px-8" },
                React.createElement("div", { className: "bg-white rounded-2xl p-10 shadow-sm border border-slate-200 text-center" },
                    ...
                    React.createElement("div", { className: "mt-6 flex flex-col sm:flex-row items-center justify-center gap-3" },
                        newId ? (React.createElement("a", { href: "#page-workflow-detail-besoin&id=".concat(newId), className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-ikaBlue text-white font-bold text-xs hover:bg-blue-600 shadow transition" }, "Voir l'expression")) : null,
    ...
    return (React.createElement("main", { className: "pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800" },
        React.createElement("div", { className: "mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 space-y-4" },
            React.createElement("nav", { className: "flex items-center gap-2 text-[11px] font-semibold text-slate-500 flex-wrap" },
                ...
                React.createElement("span", null, "/"),
                React.createElement("span", { className: "text-ikaBlue" }, isEdit ? "Modifier : ".concat(form.titre || '') : 'Nouvelle expression de besoin')),
            React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" },
                React.createElement("div", { className: "px-6 sm:px-8 py-5 border-b border-slate-100" },
                    ...
                    React.createElement("p", { className: "mt-1 text-xs text-slate-500" }, "Le besoin sera transmis au validateur indiqu\u00E9.")),
                React.createElement("form", { onSubmit: handleSubmit, className: "p-6 sm:p-8 space-y-5" },
                    ...
                    React.createElement("div", null,
                        ...
                        React.createElement("input", { type: "text", value: form.titre, onChange: function (e) { return setField('titre', e.target.value); }, required: true, placeholder: "Titre du besoin", className: "".concat(inputCls, " ").concat(errors.titre ? 'border-rose-300' : '') }),
                    ...
                    React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4" },
                        React.createElement("div", null,
                            ...
                            React.createElement("select", { value: form.priorite, onChange: function (e) { return setField('priorite', e.target.value); }, className: inputCls }, index_1.BESOIN_PRIORITES.map(function (p) { return React.createElement("option", { key: p, value: p }, p); }))),
                        React.createElement("div", null,
                            React.createElement("label", { className: "block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5" },
                                React.createElement(fa6_1.FaCalendarDays, { className: "text-ikaBlue text-[10px]" }),
                                " Date souhait\u00E9e"),
                            React.createElement("input", { type: "date", value: form.dateSouhaitee, onChange: function (e) { return setField('dateSouhaitee', e.target.value); }, required: true, className: "".concat(inputCls, " ").concat(errors.dateSouhaitee ? 'border-rose-300' : '') }),
                            errors.dateSouhaitee ? React.createElement("p", { className: "mt-1 text-[11px] font-semibold text-rose-600" }, errors.dateSouhaitee) : null)),
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-xs font-bold text-slate-700 mb-1.5" }, "Description"),
                        React.createElement("textarea", { value: form.description, onChange: function (e) { return setField('description', e.target.value); }, required: true, rows: 4, placeholder: "D\u00E9crivez votre besoin...", className: "".concat(inputCls, " ").concat(errors.description ? 'border-rose-300' : '') }),
                        errors.description ? React.createElement("p", { className: "mt-1 text-[11px] font-semibold text-rose-600" }, errors.description) : null),
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5" },
