...
export var TouteGalerie = function () {
    var _a = React.useState('all'), category = _a[0], setCategory = _a[1];
    var _b = React.useState(null), lightbox = _b[0], setLightbox = _b[1];
    var categories = __spreadArray(['all'], Array.from(new Set(GALERIE.map(function (g) { return g.category; }))), true);
    var filtered = GALERIE.filter(function (g) { return category === 'all' || g.category === category; });
