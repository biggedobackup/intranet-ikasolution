...
import { FaBullhorn, FaCalendarDays, FaLocationDot, FaNewspaper, FaUsers, FaMagnifyingGlass, FaDiagramProject, FaCubes, FaCloud, FaChartLine, FaMobileScreenButton, FaHeadset, FaGraduationCap, FaChartColumn, FaTrophy, FaCrown, FaHeart, FaComment, FaCalendarDay, FaFilePdf, FaImages, FaFolder, FaExpand, FaChevronLeft, FaChevronRight, FaChevronUp, FaChevronDown, FaArrowRight, FaXmark, FaPaperPlane, FaMobileScreen, FaPhone, FaEnvelope, FaEye } from 'react-icons/fa6';
/* ============================== DONNÉES ============================== */
var IMG = {
...
var SERVICES = [
    { icon: React.createElement(FaCubes, { className: "text-ikaBlue text-sm" }), title: 'Dev Logiciel' },
    { icon: React.createElement(FaCloud, { className: "text-emerald-600 text-sm" }), title: 'Cloud & VPS' },
    { icon: React.createElement(FaChartLine, { className: "text-purple-600 text-sm" }), title: 'Gestion ERP' },
    { icon: React.createElement(FaMobileScreenButton, { className: "text-rose-600 text-sm" }), title: 'App Mobiles' },
    { icon: React.createElement(FaHeadset, { className: "text-amber-600 text-sm" }), title: 'Infogérance' },
    { icon: React.createElement(FaGraduationCap, { className: "text-indigo-600 text-sm" }), title: 'Formation' }
];
var ANNONCES = [
...
export var Accueil = function (_a) {
    ...
    var addComment = function (e) {
        ...
        if (!val)
            return;
        setComments(__spreadArray(__spreadArray([], comments, true), [{ user: 'Vous :', text: " ".concat(val), mine: true }], false));
    ...
    var toggleAnnLike = function (id) {
        setAnnLikes(function (prev) {
            ...
            return __assign(__assign({}, prev), (_a = {}, _a[id] = { liked: !cur.liked, count: cur.liked ? cur.count - 1 : cur.count + 1 }, _a));
        });
    };
    var addAnnonceComment = function (e) {
        ...
        setAnnComments(function (prev) {
            var _a;
            return (__assign(__assign({}, prev), (_a = {}, _a[id] = __spreadArray(__spreadArray([], (prev[id] || []), true), [{ user: 'Vous :', text: " ".concat(val), mine: true }], false), _a)));
        });
        setAnnCommentCounts(function (prev) {
            var _a;
            return (__assign(__assign({}, prev), (_a = {}, _a[id] = (prev[id] || 0) + 1, _a)));
        });
                    ...
                    React.createElement("div", { className: "grid grid-cols-1 gap-2 text-xs" },
                        ...
                        React.createElement("div", { className: "flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 border border-slate-100" },
                            React.createElement("span", { className: "w-7 h-7 rounded-lg bg-blue-100 text-ikaBlue flex items-center justify-center shrink-0" },
                                React.createElement(FaPhone, { className: "text-xs" })),
                            React.createElement("div", null,
