...
import { FaClock, FaLocationDot, FaArrowLeft, FaArrowRight, FaUser, FaTag, FaCalendarDays } from 'react-icons/fa6';
...
export var DetailAgenda = function (_a) {
    var siteUrl = _a.siteUrl;
    var _b = React.useState(getAgendaIdFromHash), agendaId = _b[0], setAgendaId = _b[1];
    ...
    if (loading) {
        return (React.createElement("main", { className: "pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800" },
            React.createElement("div", { className: "mx-auto max-w-[1650px] px-4 sm:px-6 lg:px-8 text-center py-16" },
                ...
                React.createElement("p", { className: "mt-3 text-sm text-slate-500" }, "Chargement de l'agenda..."))));
    }
    var item = items.find(function (a) { return a.id === agendaId; }) || items[0];
    if (!item) {
        return (React.createElement("main", { className: "pt-6 sm:pt-8 pb-14 min-h-screen bg-slate-100 text-slate-800" },
            React.createElement("div", { className: "mx-auto max-w-[1650px] px-4 sm:px-6 lg:px-8 text-center py-16" },
                React.createElement("p", { className: "text-sm text-slate-500 font-semibold" }, "Aucun rendez-vous trouv\u00E9."),
                React.createElement("a", { href: "#page-toutes-agenda", className: "inline-block mt-4 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:bg-slate-50 transition" }, "Voir l'agenda complet"))));
    }
                    ...
                    React.createElement("div", { className: "p-5 sm:p-8" },
                        ...
                        React.createElement("div", { className: "flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 text-xs font-semibold text-slate-600" },
                            React.createElement("span", { className: "flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200" },
                                ...
                                item.day),
                            React.createElement("span", { className: "flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200" },
                                React.createElement(FaClock, { className: "text-ikaBlue" }),
                                " ",
                                item.time),
                            React.createElement("span", { className: "flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200" },
                                React.createElement(FaLocationDot, { className: "text-ikaBlue" }),
                                " ",
                                item.location)),
                ...
                React.createElement("aside", { className: "space-y-4" },
                    React.createElement("div", { className: "bg-white rounded-2xl p-5 shadow-sm border border-slate-200" },
                        React.createElement("h2", { className: "text-xs font-black uppercase tracking-wider text-slate-900 pb-3 border-b border-slate-100 mb-4 flex items-center gap-1.5" },
                            React.createElement(FaCalendarDays, { className: "text-ikaBlue text-[11px]" }),
                            " Prochains rendez-vous"),
                        React.createElement("div", { className: "grid grid-cols-1 gap-4" }, items.filter(function (a) { return a.id !== item.id; }).map(function (a) { return (React.createElement(AgendaCard, { key: a.id, item: a })); }))),
