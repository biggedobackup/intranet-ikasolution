...
export var DetailAgenda = function () {
    ...
    var item = AGENDA.find(function (a) { return a.id === agendaId; }) || AGENDA[0];
    var idx = AGENDA.findIndex(function (a) { return a.id === item.id; });
    var prev = AGENDA[(idx - 1 + AGENDA.length) % AGENDA.length];
    var next = AGENDA[(idx + 1) % AGENDA.length];
            ...
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-3 gap-4" },
                ...
                React.createElement("aside", { className: "space-y-4" },
                    React.createElement("div", { className: "bg-white rounded-2xl p-5 shadow-sm border border-slate-200" },
                        ...
                        React.createElement("div", { className: "grid grid-cols-1 gap-4" }, AGENDA.filter(function (a) { return a.id !== item.id; }).map(function (a) { return (React.createElement(AgendaCard, { key: a.id, item: a })); }))),
