...
import { FaArrowLeft, FaCalendarDays, FaUser, FaPen, FaCircleCheck, FaCircleXmark, FaHourglassHalf, FaClipboardList, FaGavel } from 'react-icons/fa6';
import { BESOINS } from '../../../services/workflow/besoins/data';
import { BESOIN_DECISION_CONFIG, applyBesoinDecision } from '../../../services/workflow/besoins/DecisionValidation';
...
export var DetailBesoin = function () {
    var _a = React.useState(function () { return BESOINS.find(function (b) { return b.id === getBesoinIdFromHash(); }) || BESOINS[0]; }), besoin = _a[0], setBesoin = _a[1];
    var _b = React.useState(null), decision = _b[0], setDecision = _b[1];
    React.useEffect(function () {
        var onHash = function () {
            var id = getBesoinIdFromHash();
            setBesoin(BESOINS.find(function (b) { return b.id === id; }) || BESOINS[0]);
            setDecision(null);
        };
    ...
    var handleDecision = function (comment, date) {
        if (!decision)
            return;
        setBesoin(function (prev) { return applyBesoinDecision(prev, decision, comment, date); });
        setDecision(null);
    };
            ...
            React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" },
                ...
                React.createElement("div", { className: "p-6 sm:p-8 space-y-5" },
                    React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4" },
                        ...
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                            React.createElement("span", { className: "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400" },
                                React.createElement(FaClipboardList, { className: "text-ikaBlue" }),
                                " Type de besoin"),
                    ...
                    React.createElement("div", { className: "pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3" },
                        ...
                        React.createElement("a", { href: "#page-workflow-liste-besoin", className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:bg-slate-50 transition" },
                            ...
                            " Retour \u00E0 la liste"))))),
        decision && (React.createElement(DecisionModal, { title: BESOIN_DECISION_CONFIG.modalTitle(decision), message: BESOIN_DECISION_CONFIG.modalMessage(besoin, decision), actionLabel: decision === 'valider' ? BESOIN_DECISION_CONFIG.validateLabel : BESOIN_DECISION_CONFIG.rejectLabel, action: decision, onConfirm: handleDecision, onCancel: function () { return setDecision(null); } }))));
};
