...
import { loadBesoin, applyBesoinDecision, deleteBesoin, loadBesoinAttachment, formatDateFR } from '../../../services/workflow/besoins/index';
...
var prioriteBadge = function (priorite) {
    switch (priorite) {
        case 'Haute': return React.createElement("span", { className: "px-2.5 py-1 rounded-full text-[10px] font-bold bg-rose-100 text-rose-600" }, priorite);
        case 'Basse': return React.createElement("span", { className: "px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-100 text-slate-500" }, priorite);
...
export var DetailBesoin = function (props) {
    ...
    var fetchBesoin = React.useCallback(function () {
        ...
        loadBesoin(siteUrl, id)
            ...
            setLoading(false);
            if (item)
                loadBesoinAttachment(siteUrl, item.id).then(setAttachment).catch(function () { return undefined; });
        })
            ...
            React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" },
                    ...
                    React.createElement("div", { className: "relative" },
                        React.createElement("div", { className: "flex items-start justify-between gap-4 flex-wrap" },
                            React.createElement("div", null,
                                React.createElement("h1", { className: "text-xl sm:text-2xl font-black text-ikaBlueDark" }, besoin.titre),
                ...
                React.createElement("div", { className: "p-6 sm:p-8 space-y-5" },
                    React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4" },
                        ...
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                            React.createElement("span", { className: "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400" },
                                ...
                                " Date souhait\u00E9e"),
                            React.createElement("p", { className: "mt-1.5 text-sm font-bold text-slate-800" }, formatDateFR(besoin.dateSouhaitee))),
                        React.createElement("div", { className: "rounded-xl border border-slate-100 p-4 bg-slate-50/60" },
                    ...
                    isEnAttente && isValidateur ? (React.createElement("div", { className: "rounded-xl border border-amber-200 bg-amber-50/60 p-5 space-y-3" },
                        ...
                        React.createElement("p", { className: "text-xs text-amber-700/80 leading-relaxed" }, "Cette expression de besoin est en attente. Vous pouvez la valider ou la rejeter avec un commentaire."),
                        ...
                        React.createElement("p", { className: "text-xs font-semibold text-slate-700" },
                            besoin.statut === 'Approuvé' ? (React.createElement("span", { className: "flex items-center gap-1.5 text-emerald-700" },
                                React.createElement(FaCircleCheck, null),
                                " ",
                                BESOIN_DECISION_CONFIG.validateVerb)) : (React.createElement("span", { className: "flex items-center gap-1.5 text-rose-600" },
                                React.createElement(FaCircleXmark, null),
                                " ",
                                BESOIN_DECISION_CONFIG.rejectVerb)),
                            besoin.dateDecision ? React.createElement("span", { className: "text-slate-400 font-normal" },
                                " \u2014 le ",
                                formatDateFR(besoin.dateDecision)) : null),
                    ...
                    React.createElement("div", { className: "pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3" },
                        ...
                        React.createElement("a", { href: "#page-workflow-liste-besoin", className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:bg-slate-50 transition" },
                            React.createElement(FaArrowLeft, null),
                            " Retour \u00E0 la liste"))))),
        decision ? (React.createElement(DecisionModal, { title: BESOIN_DECISION_CONFIG.modalTitle(decision), message: BESOIN_DECISION_CONFIG.modalMessage(besoin, decision), actionLabel: deciding ? 'Enregistrement...' : (decision === 'valider' ? BESOIN_DECISION_CONFIG.validateLabel : BESOIN_DECISION_CONFIG.rejectLabel), action: decision, onConfirm: handleDecision, onCancel: function () { return setDecision(null); } })) : null,
        confirmDelete ? (React.createElement(ConfirmDelete, { title: "Supprimer l'expression", message: "Voulez-vous vraiment supprimer l'expression de besoin \u00AB ".concat(besoin.titre, " \u00BB de ").concat(besoin.demandeur, " ? Cette action est irr\u00E9versible."), onConfirm: handleDelete, onCancel: function () { return !deleting && setConfirmDelete(false); } })) : null));
};
export default DetailBesoin;
//# sourceMappingURL=DetailBesoin.js.map