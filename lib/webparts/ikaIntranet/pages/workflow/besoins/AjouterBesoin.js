...
import { BESOIN_PRIORITES, loadBesoin, createBesoin, updateBesoin, loadBesoinAttachment, uploadBesoinAttachment, removeBesoinAttachment } from '../../../services/workflow/besoins/index';
import { getCurrentUserName, getCurrentUserEmail, isBlank, isValidEmail } from '../../../services/shared/index';
...
export var AjouterBesoin = function (props) {
    ...
    React.useEffect(function () {
        ...
        if (isEdit && id) {
            ...
            loadBesoin(siteUrl, id)
                ...
                setLoading(false);
                loadBesoinAttachment(siteUrl, id).then(function (att) { if (!cancelled)
    ...
    var handleSubmit = function (e) {
        ...
        if (isEdit && id) {
            updateBesoin(siteUrl, id, payload).then(function (ok) { return afterSave(ok, id); }).catch(function () { return finish(false); });
        }
        else {
            createBesoin(siteUrl, payload).then(function (createdId) { return afterSave(!!createdId, createdId); }).catch(function () { return finish(false); });
        }
    };
                ...
                React.createElement("form", { onSubmit: handleSubmit, className: "p-6 sm:p-8 space-y-5" },
                    ...
                    React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4" },
                        React.createElement("div", null,
                            ...
                            React.createElement("select", { value: form.priorite, onChange: function (e) { return setField('priorite', e.target.value); }, className: inputCls }, BESOIN_PRIORITES.map(function (p) { return React.createElement("option", { key: p, value: p }, p); }))),
                        React.createElement("div", null,
                            React.createElement("label", { className: "block text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5" },
