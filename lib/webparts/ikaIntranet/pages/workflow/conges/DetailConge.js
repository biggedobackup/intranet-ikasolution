...
import { FaArrowLeft, FaCalendarDays, FaUser, FaPen, FaCircleCheck, FaCircleXmark, FaHourglassHalf, FaGavel } from 'react-icons/fa6';
import { CONGES } from '../../../services/workflow/conges/data';
import { CONGE_DECISION_CONFIG, applyCongeDecision } from '../../../services/workflow/conges/DecisionValidation';
...
export var DetailConge = function () {
    var _a = React.useState(function () { return CONGES.find(function (c) { return c.id === getCongeIdFromHash(); }) || CONGES[0]; }), conge = _a[0], setConge = _a[1];
    var _b = React.useState(null), decision = _b[0], setDecision = _b[1];
    React.useEffect(function () {
        var onHash = function () {
            var id = getCongeIdFromHash();
            setConge(CONGES.find(function (c) { return c.id === id; }) || CONGES[0]);
            setDecision(null);
        };
    ...
    var handleDecision = function (comment, date) {
        if (!decision)
            return;
        setConge(function (prev) { return applyCongeDecision(prev, decision, comment, date); });
        setDecision(null);
    };
                ...
                React.createElement("div", { className: "p-6 sm:p-8 space-y-5" },
                    ...
                    React.createElement("div", { className: "pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3" },
                        ...
                        React.createElement("a", { href: "#page-workflow-liste-conge", className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:bg-slate-50 transition" },
                            ...
                            " Retour \u00E0 la liste"))))),
        decision && (React.createElement(DecisionModal, { title: CONGE_DECISION_CONFIG.modalTitle(decision), message: CONGE_DECISION_CONFIG.modalMessage(conge, decision), actionLabel: decision === 'valider' ? CONGE_DECISION_CONFIG.validateLabel : CONGE_DECISION_CONFIG.rejectLabel, action: decision, onConfirm: handleDecision, onCancel: function () { return setDecision(null); } }))));
};
