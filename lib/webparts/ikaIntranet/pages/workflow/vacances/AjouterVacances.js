...
import { FaArrowLeft, FaCalendarDays, FaUser, FaUserCheck, FaPaperPlane, FaRotate, FaCircleCheck, FaLocationDot, FaTriangleExclamation, FaPaperclip } from 'react-icons/fa6';
import { loadVacance, createVacance, updateVacance, loadVacanceAttachment, uploadVacanceAttachment, removeVacanceAttachment } from '../../../services/workflow/vacances/index';
...
export var AjouterVacances = function (props) {
    ...
    React.useEffect(function () {
        ...
        if (isEdit && id) {
            ...
            loadVacance(siteUrl, id)
                ...
                setLoading(false);
                loadVacanceAttachment(siteUrl, id).then(function (att) { if (!cancelled)
    ...
    var handleSubmit = function (e) {
        ...
        if (isEdit && id) {
            updateVacance(siteUrl, id, payload).then(function (ok) { return afterSave(ok, id); }).catch(function () { return finish(false); });
        }
        else {
            createVacance(siteUrl, payload).then(function (createdId) { return afterSave(!!createdId, createdId); }).catch(function () { return finish(false); });
        }
    };
