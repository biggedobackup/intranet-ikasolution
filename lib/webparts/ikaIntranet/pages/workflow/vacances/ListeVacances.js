...
export var ListeVacances = function () {
    ...
    var _d = React.useState(VACANCES), items = _d[0], setItems = _d[1];
    var _e = React.useState(null), deleteItem = _e[0], setDeleteItem = _e[1];
    var statuses = __spreadArray(['all'], Array.from(new Set(items.map(function (i) { return i.statut; }))), true);
    var filtered = items.filter(function (i) {
        var q = search.toLowerCase();
        var matchesSearch = i.titre.toLowerCase().includes(q) || i.demandeur.toLowerCase().includes(q) || i.destination.toLowerCase().includes(q);
