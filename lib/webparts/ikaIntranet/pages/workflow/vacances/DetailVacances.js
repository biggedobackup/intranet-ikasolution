...
import { FaArrowLeft, FaCalendarDays, FaUser, FaPen, FaCircleCheck, FaCircleXmark, FaHourglassHalf, FaPlaneDeparture, FaGavel } from 'react-icons/fa6';
import { VACANCES } from '../../../services/workflow/vacances/data';
import { VACANCE_DECISION_CONFIG, applyVacanceDecision } from '../../../services/workflow/vacances/DecisionValidation';
...
export var DetailVacances = function () {
    var _a = React.useState(function () { return VACANCES.find(function (v) { return v.id === getVacanceIdFromHash(); }) || VACANCES[0]; }), vacance = _a[0], setVacance = _a[1];
    var _b = React.useState(null), decision = _b[0], setDecision = _b[1];
    React.useEffect(function () {
        var onHash = function () {
            var id = getVacanceIdFromHash();
            setVacance(VACANCES.find(function (v) { return v.id === id; }) || VACANCES[0]);
            setDecision(null);
        };
    ...
    var handleDecision = function (comment, date) {
        if (!decision)
            return;
        setVacance(function (prev) { return applyVacanceDecision(prev, decision, comment, date); });
        setDecision(null);
    };
                ...
                React.createElement("div", { className: "p-6 sm:p-8 space-y-5" },
                    ...
                    React.createElement("div", { className: "pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3" },
                        ...
                        React.createElement("a", { href: "#page-workflow-liste-vacances", className: "inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:bg-slate-50 transition" },
                            ...
                            " Retour \u00E0 la liste"))))),
        decision && (React.createElement(DecisionModal, { title: VACANCE_DECISION_CONFIG.modalTitle(decision), message: VACANCE_DECISION_CONFIG.modalMessage(vacance, decision), actionLabel: decision === 'valider' ? VACANCE_DECISION_CONFIG.validateLabel : VACANCE_DECISION_CONFIG.rejectLabel, action: decision, onConfirm: handleDecision, onCancel: function () { return setDecision(null); } }))));
};
