...
import { ABSENCE_TYPES, loadAbsence, createAbsence, updateAbsence, loadAbsenceAttachment, uploadAbsenceAttachment, removeAbsenceAttachment } from '../../../services/workflow/absences/index';
...
var emptyForm = { titre: '', type: ABSENCE_TYPES[0], dateDebut: '', dateFin: '', jours: '', motif: '', validateurEmail: '' };
function formFromExisting(existing) {
    return {
        titre: existing.titre,
        type: existing.type || ABSENCE_TYPES[0],
...
export var AjouterAbsence = function (props) {
    ...
    React.useEffect(function () {
        ...
        if (isEdit && id) {
            ...
            loadAbsence(siteUrl, id)
                ...
                setLoading(false);
                loadAbsenceAttachment(siteUrl, id).then(function (att) { if (!cancelled)
    ...
    var handleRemoveAttachment = function () {
        if (!siteUrl || !id || !existingAttachment)
            return;
        removeAbsenceAttachment(siteUrl, id, existingAttachment.fileName)
    ...
    var handleSubmit = function (e) {
        ...
        var afterSave = function (ok, targetId) {
            if (ok && targetId && file) {
                uploadAbsenceAttachment(siteUrl, targetId, file)
        ...
        if (isEdit && id) {
            updateAbsence(siteUrl, id, payload).then(function (ok) { return afterSave(ok, id); }).catch(function () { return finish(false); });
        }
        else {
            createAbsence(siteUrl, payload).then(function (createdId) { return afterSave(!!createdId, createdId); }).catch(function () { return finish(false); });
        }
    };
            ...
            React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" },
                ...
                React.createElement("form", { onSubmit: handleSubmit, className: "p-6 sm:p-8 space-y-5" },
                    ...
                    React.createElement("div", null,
                        ...
                        React.createElement("select", { value: form.type, onChange: function (e) { return setField('type', e.target.value); }, className: inputCls }, ABSENCE_TYPES.map(function (t) { return React.createElement("option", { key: t, value: t }, t); }))),
