...
import { FaArrowLeft, FaArrowRight, FaCrown, FaHeart, FaComment, FaTrophy, FaCalendarDays } from 'react-icons/fa6';
...
export var DetailEmployeMois = function () {
    ...
    var employe = EMPLOYES_MOIS.find(function (e) { return e.id === employeId; }) || EMPLOYES_MOIS[0];
    var idx = EMPLOYES_MOIS.findIndex(function (e) { return e.id === employe.id; });
    var prev = EMPLOYES_MOIS[(idx - 1 + EMPLOYES_MOIS.length) % EMPLOYES_MOIS.length];
    var next = EMPLOYES_MOIS[(idx + 1) % EMPLOYES_MOIS.length];
